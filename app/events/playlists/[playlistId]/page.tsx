...
export async function generateMetadata({ params }: PlaylistPageProps): Promise<Metadata> {
  ...
  if (!isEventPlaylistId(playlistId)) {
    return {
      ...
      description: "Watch recorded event videos from The Unfiltered IITian.",
    };
  }
  ...
  return {
    title: playlist?.title ?? "Event Playlist",
    description:
      playlist?.description ??
      "Watch recorded event videos from The Unfiltered IITian.",
  };
}

export default async function EventPlaylistPage({ params }: PlaylistPageProps) {
  const { playlistId } = await params;
    ...
    <div className={styles.page}>
      <section className={styles.hero}>
        <div className={styles.heroCopy}>
          <span className={styles.eyebrow}>Event Playlist</span>
          ...
          <div className={styles.linkRow}>
            ...
            <Link className={styles.secondaryLink} href="/events">
              Back to events
            </Link>
          </div>
        </div>
        ...
        <div className={styles.heroVisual}>
          ...
          <div className={styles.playlistHeroMeta}>
            ...
            <p>Choose any event below and continue watching on YouTube.</p>
          </div>
        </div>
      </section>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeader}>
          ...
          <h2>Watch live event recordings one video at a time.</h2>
          <p>
            Each button opens the selected recording directly on YouTube.
          </p>
        </div>
